...
void Window::handleInput(Scene &scene)
{
    SDL_Event event;
        if (SDL_PollEvent(&event))
        {
            switch (event.type)
            {
                ...
                case SDL_MOUSEWHEEL:
                {
                    if (event.wheel.y > 0)
                    {
                        for (auto &object :scene.d_objects)
                        {
                ...
                case SDL_MOUSEMOTION:
                {
                  if (d_left_mouse_clicked)
                  {
                    ...
                    if (event.motion.xrel  < 0)
                        scene.d_objects.at(0).rotationMatrix.rotateSelf(0.0f, -1.0f, 0);
                    else if (event.motion.xrel > 0)
                        scene.d_objects.at(0).rotationMatrix.rotateSelf(0.0f, 1.0f, 0);

                    if (event.motion.yrel < 0)
                        scene.d_objects.at(0).rotationMatrix.rotateSelf(-1.0f, 0.0f, 0);
                    else if (event.motion.yrel > 0)
                        scene.d_objects.at(0).rotationMatrix.rotateSelf(1.0f, 0.0f, 0.0f);
                  }
                    break;
